...
import { equipmentSchema, type EquipmentFormValues } from "@/domain/schemas";
import { equipmentQuestions } from "@/domain/questions";
...
interface Props {
  ...
  onNext: (values: EquipmentFormValues) => void;
  onPrev: () => void;
  onSaveConfig: (values: Partial<EquipmentFormValues>) => void;
  onAbandon: () => void;
}
...
export function EquipmentForm({
  ...
  const onSubmit = (data: EquipmentFormValues) => onNext(data);
  ...
  return (
    <form
      ...
      className="space-y-4"
    >
      {equipmentQuestions.map((q, index) => {
        const error = form.formState.errors[q.id as keyof EquipmentFormValues];
        const currentValue = form.watch(q.id as keyof EquipmentFormValues);
        ...
        return (
          ...
          >
            <CardContent className="p-4 sm:p-6 lg:p-8">
              ...
              <RadioGroup
                onValueChange={(val) => {
                  form.setValue(q.id as keyof EquipmentFormValues, val as any, {
                    shouldValidate: true,
                  });
                  onSaveConfig({ ...form.getValues(), [q.id]: val } as any);
                }}
                defaultValue={
                  defaultValues?.[q.id as keyof EquipmentFormValues] as string
                }
      ...
      <div className="sticky bottom-4 z-50 w-full mt-8 pointer-events-none">
        <div className="bg-background/85 backdrop-blur-md px-4 py-3 rounded-2xl border shadow-xl flex justify-between items-center gap-2 pointer-events-auto">
          ...
          <div className="flex gap-2 items-center">
            ...
            <Button
              type="submit"
              size="lg"
              className="cursor-pointer font-heading font-semibold tracking-wide shadow-lg shadow-primary/20 px-5 sm:px-10 gap-2"
            >
