...
function AnswerBadge({ value }: { value: AnswerValue | undefined }) {
  ...
  return (
    <Badge className={`font-body text-xs ${config.className}`}>{config.label}</Badge>
  );
}
...
export function ResultDetailView() {
    ...
    return (
      <div className="bg-background rounded-2xl shadow-sm border p-8 text-center space-y-4">
        ...
        <p className="font-body text-muted-foreground">
          El registro solicitado no existe o fue eliminado del historial local.
        </p>
        <a href="/">
  ...
  return (
    <div className="bg-background rounded-2xl shadow-sm border p-4 sm:p-8 space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Question lists by section */}
      {SECTIONS.map((section, idx) => (
        <div key={section.key}>
          <QuestionList
            title={section.label}
            questions={ALL_SECTIONS_QUESTIONS[section.key as SectionKey]}
            answers={(result.answers[section.key as SectionKey] as Record<string, AnswerValue>) ?? {}}
          />
        ...
        <Card className="border-sky-500/30 bg-sky-500/5">
          <CardHeader>
            <div className="flex items-center gap-2">
              ...
              <CardTitle className="font-heading text-sky-700 dark:text-sky-400 text-lg">
                Plan de Acción
              </CardTitle>
            </div>
          </CardHeader>
